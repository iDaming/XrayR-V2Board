...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/iDaming/XrayR/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 XrayR 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 XrayR 版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 XrayR 最新版本：${last_version}，开始安装"
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux-64.zip https://github.com/iDaming/XrayR/releases/download/${last_version}/XrayR-linux-64.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/iDaming/XrayR/releases/download/${last_version}/XrayR-linux-64.zip"
    ...
    file="https://github.com/iDaming/XrayR-V2Board/raw/main/XrayR.service"
    ...
    # Writing json
    echo "正在尝试写入配置文件..."
    wget https://cdn.jsdelivr.net/gh/iDaming/XrayR-V2Board/config.yml -O /etc/XrayR/config.yml
